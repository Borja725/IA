...
# Leer el archivo CSV
df = pd.read_csv("DATASETS\\Dataset_400_Mod_v3.csv").head(100)
  ...
  "Verbatims Negativos": {
    "Entregas": {
      ...
      "0103": {
        ...
        "details": "Este código cubre errores relacionados con la exactitud del pedido, tales como recibir productos incorrectos, faltantes, o adicionales. No incluye problemas de entrega, que se manejan con otros códigos."
      },
      "0104": {
    ...
    "Verbatims Positivos": {
      ...
      "Otros Positivos": {
        "9995": {
          ...
          "conditions": ["comentarios positivos", "no clasificados", "general", "aprecio", "Errores"],
...
prompt_template = """
Instrucciones:
1. Descompón el comentario en conclusiones semánticas como nombre semantic_conclusions, usando metodologías avanzadas, asegurando que se conserve el contexto original.
2. Incluye información como:
   2.1 Canal de comunicación.
   2.2 Lugar de interacción.
3. Cualquier otro dato relevante.
4. Si el canal de comunicación no está claro, indícalo como "telefonico".
5. Si el lugar no está claro, indícalo como "a distancia".
6. La salida debe estar en formato JSON, ademas de que no este dentro de un bloque de codigo JSON, si no que este en texto plano.
7. Incluye metadatos adicionales sobre los temas y sentimientos involucrados.
8. No hagas suposiciones, limítate únicamente al comentario proporcionado.
9. Si "sentiment" del JSON es "positive" usarás únicamente los "positive_sentiment" del plan de códigos. 
10. Si "sentiment" del JSON es "negative" usarás unicamente los "negative_sentiment" del plan de códigos.
11. Las categorys se sacan dado este plan de codificacion:
...
Por ejemplo:
Dado el comentario: "El servicio de atención al cliente fue excelente, me ayudaron con mi problema rápidamente. Además, la entrega fue rápida y el producto llegó en perfectas condiciones."
Salida JSON esperada:

{{
    "context": {{
        "location": "a distancia",
        "channel_of_communication": "telefónico",
        "additional_info": {{
            "customer_id": "optional_customer_identifier",
            "order_id": "optional_order_identifier"
        }}
    }},
    "units": [
        {{
            "text": "El servicio de atención al cliente fue excelente",
            "semantic_conclusion": "Evaluación positiva del servicio de atención al cliente.",
            "themes": [
                "Customer Service",
                "Staff Interaction"
            ],
            ...
            "category": "BTQ - 97",
            "details": {{
                "impact": "Alta satisfacción del cliente.",
                "responsible_party": "Departamento de atención al cliente.",
                "expected_action": "Reconocimiento y recompensa al personal eficiente."
            }}
        }},
        {{
            "text": "me ayudaron con mi problema rápidamente",
            "semantic_conclusion": "Resolución rápida y eficiente del problema del cliente.",
            "themes": [
                "Customer Support",
                "Issue Resolution"
            ],
            ...
            "details": {{
                "impact": "Alta satisfacción del cliente.",
                "responsible_party": "Departamento de atención al cliente.",
                "expected_action": "Mantener y mejorar la eficiencia en la resolución de problemas."
            }}
        }},
        {{
            "text": "Además, la entrega fue rápida y el producto llegó en perfectas condiciones.",
            "semantic_conclusion": "Evaluación positiva de la rapidez y estado de la entrega.",
            "themes": [
                "Delivery",
                "Product Condition"
            ],
            "sentiment": "positive",
            "category": "Entregas - 91",s
            "details": {{
                "impact": "Alta satisfacción del cliente.",
                "responsible_party": "Departamento de logística y entrega.",
                "expected_action": "Mantener y mejorar los estándares de entrega."
            }}
        }}
    ]
}}

Siguiendo las instrucciones detalladas, genera ese JSON para el siguiente comentario, en texto plano, no pongas ```:
...
# Crear la plantilla del prompt con la clave 'text'
prompt = PromptTemplate(template=prompt_template, input_variables=["text", "json_definition"])

# Crear la cadena LLM
chain = LLMChain(llm=llm, prompt=prompt)

# Función para generar el JSON usando .invoke
def generate_json(text):
    try:
        # Llamar al modelo con la variable 'text'
        result = chain.invoke({"text": text, "json_definition": json_definition})
        json_output = result.get('text', '')
        print("Resultado del modelo:", json_output)  # Añadido para depuración
        return json_output
    except KeyError as e:
        print(f"Error en el resultado del modelo: {e}")
        return ""

# Procesar los verbatims y generar el JSON
json_results = []

for verb in tqdm(zip(verbatims), total=len(verbatims)):
    json_output = generate_json(verb)
    try:
        json_data = json.loads(json_output)
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON for verbatim: {verb}")
        print("Error detalle:", e)
        print("Salida JSON cruda:", json_output)
        json_data = {}
    json_results.append({
        "VERBATIM":verb, 
        "PREDICCION":json_data})

# Guardar los resultados en un archivo JSON
with open('verbatim_predictions_llama3_dataset_Ultimo.json', 'w', encoding="utf-8") as f:
